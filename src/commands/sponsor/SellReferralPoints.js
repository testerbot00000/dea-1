...
class SellReferralPoints extends patron.Command {
  ...
  async run(msg, args) {
    ...
    await args.member.DM(msg.author.tag.boldify() + ' has offered you ' + referralPoints + ' in exchange for ' + price + '. To accept this offer, reply with "' + key + '" within the next 5 minutes.', { guild: msg.guild });
    ...
    if (result.size >= 1) {
      ...
      if (seller.points < args.points) {
        return args.member.DMError(msg.author.tag.boldify() + ' does not have ' + referralPoints + '.', { guild: msg.guild });
      ...
      await args.member.DM('You have successfully purchased ' + referralPoints + ' in exchange for ' + price + '.', { guild: msg.guild });
      return msg.author.tryDM('You have successfully sold ' + referralPoints + ' for ' + price + '.', { guild: msg.guild });
    }

    return msg.author.tryDM(args.member.user.tag.boldify() + ' has not responded to your referral points offer.', { guild: msg.guild });
  }
}
