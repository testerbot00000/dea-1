class Items {
  ...
  async crateItems(crateId) {
    const result = await this.db.pool.query('SELECT d.names, d.crate_odds, d.id FROM crate_items i JOIN item_data d ON d.id = i.item_id WHERE i.crate_id = $1 ORDER BY d.names[1];', [crateId]);

    return result.rows;
  }

  async gunBullets(gunId) {
    const result = await this.db.pool.query('SELECT i.names FROM gun_bullets g JOIN item_data i ON g.bullet_id = i.id WHERE g.gun_id = $1 ORDER BY i.names[1];', [gunId]);
  ...
  async inventory(userId, guildId) {
    const result = await this.db.pool.query('SELECT d.names, quantity FROM items i JOIN item_data d ON d.id = i.data_id WHERE (user_id, guild_id) = ($1, $2) ORDER BY d.names[1];', [userId, guildId]);
